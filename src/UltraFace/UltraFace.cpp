...
UltraFace::UltraFace(const std::string &mnn_path, int input_width,
  ...
  input_tensor =
      ultraface_interpreter->getSessionInput(ultraface_session, nullptr);
}
...
int UltraFace::detect(cv::Mat &raw_image, std::vector<FaceInfo> &face_list) {
  ...
  string boxes = "boxes";
  MNN::Tensor *tensor_scores = ultraface_interpreter->getSessionOutput(
      ultraface_session, scores.c_str());
  MNN::Tensor *tensor_boxes =
      ultraface_interpreter->getSessionOutput(ultraface_session, boxes.c_str());
...
void UltraFace::generateBBox(std::vector<FaceInfo> &bbox_collection,
  ...
  for (int i = 0; i < num_anchors; i++) {
    if (scores->host<float>()[i * 2 + 1] > score_threshold) {
      FaceInfo rects;
      float x_center =
          boxes->host<float>()[i * 4] * center_variance * priors[i][2] +
          priors[i][0];
      float y_center =
          boxes->host<float>()[i * 4 + 1] * center_variance * priors[i][3] +
          priors[i][1];
      float w =
          exp(boxes->host<float>()[i * 4 + 2] * size_variance) * priors[i][2];
      float h =
          exp(boxes->host<float>()[i * 4 + 3] * size_variance) * priors[i][3];
