...
public:
   ...
   */
  static ImageProcess *create(const Config &config,
   ...
   */
  static ImageProcess *
  create(const ImageFormat sourceFormat = RGBA,
         const ImageFormat destFormat = RGBA, const float *means = nullptr,
         const int meanCount = 0, const float *normals = nullptr,
   ...
   */
  inline const Matrix &matrix() const { return mTransform; }
   ...
   * @param stride    number of elements per row. eg: 100 width RGB contains at
   * least 300 elements.
   * @param dest      given tensor.
   * @return result code.
   */
  ErrorCode convert(const uint8_t *source, int iw, int ih, int stride,
                    Tensor *dest);
   ...
   * @param outputBpp      output bpp, if 0, set as the save and
   * config.destFormat.
   * @param outputStride  output stride, if 0, set as ow * outputBpp.
   * @param type  Only support halide_type_of<uint8_t> and
   * halide_type_of<float>.
   * @return result code.
   */
  ErrorCode convert(const uint8_t *source, int iw, int ih, int stride,
                    void *dest, int ow, int oh, int outputBpp = 0,
                    int outputStride = 0,
  ...
  static Tensor *createImageTensor(halide_type_t type, int w, int h, int bpp,
                                   void *p = nullptr);
