...
public:
   ...
   */
  virtual bool onWaitFinish() { return true; }
  /**
   * @brief load GPU library resource.
   * @param library   loading load GPU library.
   * @return success or not.
   */
  virtual bool onLoadLibrary(const GpuLibrary *library) { return false; }
...
public:
   ...
   */
  virtual bool onAllocateBuffer() { return true; }
 ...
 * @return true if backend creator for given forward type was not registered
 * before, false otherwise.
 */
MNN_PUBLIC bool MNNInsertExtraBackendCreator(MNNForwardType type,
                                             const BackendCreator *creator,
